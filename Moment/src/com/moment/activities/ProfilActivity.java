...
public class ProfilActivity extends SherlockActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("URMMMMM", AppMoment.getInstance().user.getPictureProfileUrl());
        
        // Si elle existe on recupere l'image du profil
        if(AppMoment.getInstance().user.getPictureProfileUrl()!=null) {
        	Log.d("URMMMMM", AppMoment.getInstance().user.getPictureProfileUrl());
        	
        	AsyncHttpClient client = new AsyncHttpClient();
        	String[] allowedContentTypes = new String[] { "image/png", "image/jpeg" };
        	client.get(AppMoment.getInstance().user.getPictureProfileUrl(), new BinaryHttpResponseHandler(allowedContentTypes) {
